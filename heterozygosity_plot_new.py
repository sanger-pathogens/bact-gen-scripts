...
def main():
	...
	group.add_option("-o", "--output", action="store", dest="output", help="output file name (file type will be guessed from the name. e.g. plot.pdf will be saved in pdf format) if no output name is given, the plot will be shown on the screen", default="", metavar="file name ")
...
#		DPratiof=0.0
#		DPratior=0.0
		if (DP[0]+DP[1]+DP[2]+DP[3])>=options.depth and DP[0]+DP[2]>=options.stranddepth and DP[1]+DP[3]>=options.stranddepth and ((DP[0]+DP[1]>2) and (DP[2]+DP[3]>2)):
...
#		
	...
	#pylab.axvspan((1.0-options.ratio)*100, (options.ratio)*100, facecolor='r', alpha=0.2, hatch='/')
	n, bins, patches = pylab.hist(data, bins=25, log=True, label=label, alpha=0.7, range=(0,50)) 
	#pylab.legend(loc="best")
	pylab.axis(ymin=0.1)
	...
	pylab.title("Base heterozygosity plot of "+options.bcf) 
	pylab.xlabel("%age of reads with minor allele") 
	pylab.ylabel("Frequency")
	if options.output=="":
		pylab.show()
	else:
