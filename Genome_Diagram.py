...
def main():
	...
	group.add_option("-H", "--height", action="store", dest="height", help="Track height [default= %default]", default=1, metavar="int", type="int")
...
#	parser.add_option("-r", "--reference", action="store", dest="alignment", help="multifasta file corresponding to embl files (necesary if embl file isn't in true embl format). sequence names must correspond to the embl file prefix.", default="", metavar="FILE")
	...
	group.add_option("-S", "--label_size", action="store", dest="labelsize", help="Size of labels (CDS names and scales) [default= %default]", default=4, metavar="int", type="int")
...
#			options.plotmax=(numpy.std(data[options.beginning:options.end])*numpy.std(data[options.beginning:options.end]))+numpy.mean(data[options.beginning:options.end])
	...
	if options.automax:
			...
			if options.plotmax>numpy.max(mwresults):
				options.plotmax=numpy.max(mwresults)
...
#	graphdata[0]=(graphdata[0][1],14.887)
#	graphdata[-1]=(graphdata[-1][1],-14.887)
	...
	if options.scale:
		gdt = GenomeDiagram.Track("Plot Track", scale_fontsize=int(options.labelsize), scale_fontangle=0, scale_ticks=0, scale=1, greytrack=greytracks, greytrack_labels=0, scale_largetick_interval=len(plotdata), height=options.height)
	else:
		gdt = GenomeDiagram.Track("Plot Track", scale_fontsize=int(options.labelsize), scale_fontangle=0, scale_ticks=0, scale=0, greytrack=greytracks, greytrack_labels=0, scale_largetick_interval=len(plotdata), height=options.height)
	gdgs = GenomeDiagram.GraphSet("Plot")
	gdgs.new_graph(graphdata, name="Graph Name", style=options.plottype, color=colors.red, altcolour=colors.blue, linewidth=1)
	gdt.add_set(gdgs)
...
def add_bam_to_diagram(filename, track_number, log=False):		
	...
	if len(plotdata)==0:
		plotdata=[0]*(500+options.beginning)
...
#				break
	...
	gd_track_for_features = gd_diagram.new_track(track_number, scale=0, height=options.height)
...
def add_embl_to_diagram(record, track_number, filetype):
	...
	if options.kyte==True:
		...
		gdt = GenomeDiagram.Track("Kyte Doolittle Track", scale_fontsize=int(options.labelsize), scale_fontangle=0, scale_ticks=0, scale=1, greytrack=greytracks, greytrack_labels=0, scale_largetick_interval=len(record.seq), height=options.height*2)
		gdgs = GenomeDiagram.GraphSet("Plot")
	...
	if options.gc_dev==True:
			
		graphdata=seq_2_moving_window_gc_deviation(record.seq, windowsize)
		graphdata[0]=(1,(float(graphdata[1][1])+float(graphdata[-1][1]))/2)
		
		gdt = GenomeDiagram.Track("GC deviation Track", scale_fontsize=int(options.labelsize), scale_fontangle=0, scale_ticks=0, scale=1, greytrack=greytracks, greytrack_labels=0, scale_largetick_interval=len(record.seq), height=options.height)
		gdgs = GenomeDiagram.GraphSet("Plot")
		gdgs.new_graph(graphdata, name="GC deviation", style=options.plottype, color=colors.violet, altcolour=colors.purple)
		gdt.add_set(gdgs)
		gd_diagram.add_track(gdt, track_number)
		track_number=track_number+1
	...
	if options.gc==True:
		...
		gdt = GenomeDiagram.Track("GC content Track", scale_fontsize=int(options.labelsize), scale_fontangle=0, scale_ticks=0, scale=1, greytrack=greytracks, greytrack_labels=0, scale_largetick_interval=len(graphdata), height=options.height)
		gdgs = GenomeDiagram.GraphSet("Plot")
	...
	for x, feature in enumerate(record.features):
		...
		if feature.type not in  incfeatures or feature.location.nofuzzy_end<options.beginning or (feature.location.nofuzzy_start>options.end and options.end!=-1):#,"tRNA","repeat_unit"] :
		...
		if len(colourline.split())==1:
			colour=translator.artemis_color(colourline.replace('"',''))
	...
	track_number=track_number+1
	if options.labels==True:# and options.circular==True:
...
def add_tab_to_diagram(filename, track_number):
	...
	for line in open(filename,"r"):
		...
		#if len(words)==3 and words[1].lower() in ["snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination"]  and int(words[2].split("..")[0])>=options.beginning and (options.end==-1 or int(words[2].split("..")[0])<=options.end) and (options.end==-1 or (len(words[2].split(".."))==2 and int(words[2].split("..")[1])<=options.end)):
		#if len(words)==3 and words[1].lower() in ["snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination", "feature", "blastn_hit"]  and (options.end==-1 or int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[0])<=options.end) and (options.end==-1 or (len(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split(".."))==2 and int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[1])>=options.beginning)):
		loclist=[]
		if len(words)==3 and words[1].lower() in ["i", "d", "li", "del", "snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination", "feature", "blastn_hit", "fasta_record"]:
			loclist.append(int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[0]))
			if len(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split(".."))==2:
				loclist.append(int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[1]))
			else:
				loclist.append(int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[0]))
		...
		if len(words)==3 and words[1].lower() in ["i", "d", "li", "del", "snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination", "feature", "blastn_hit", "fasta_record"]  and (options.end==-1 or loclist[0]<=options.end) and (options.end==-1 or loclist[0]>=options.beginning):	
#			try: print feature, colour
#			except NameError: pass #print "here"
			try: features.append((feature,colour))
			except NameError: pass #print "here"
			
			if words[1].lower()=="snp":
				colour='2'
			elif words[1].lower()=="misc_feature":
				colour='4'
			elif words[1].lower()=="cds":
				colour='5'
			elif words[1].lower()=="gene":
				colour='5'
			elif words[1].lower()=="recombination":
				colour='3'
			else:
				colour='14'
			
				
			position=words[2].replace("complement(","").replace(")","").replace(">","").replace("<","")
		...
		elif len(words)==2 and words[1].split("=")[0][1:] in ["color", "colour"]:
			colour=words[1].split("=")[1].replace('"','')
		elif len(words)>2 and words[1].split("=")[0][1:] in ["color", "colour"]:
			colourbits=" ".join(words[1:]).split("=")[1].replace('"','').split()
			if len(colourbits)!=3:
				print "Can't understand colour code!"
				sys.exit()
			else:
				colour=(int(colourbits[0]),int(colourbits[1]),int(colourbits[2]))
	...
	if track_number %2:
		gd_track_for_features = gd_diagram.new_track(track_number, scale=0, height=options.height, greytrack=greytracks, greytrack_labels=0)
	else:
		gd_track_for_features = gd_diagram.new_track(track_number, scale=0, height=options.height)
	gd_feature_set = gd_track_for_features.new_set()
...
def add_ordered_tab_to_diagram(filename, track_number, nameorder):
	...
	for x, line in enumerate(filelines):
		...
		loclist=[]
		if len(words)==3 and words[1].lower() in ["snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination", "feature", "blastn_hit", "fasta_record"]:
			loclist.append(int(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[0]))
			if len(words[2].replace("complement(","").replace(")","").replace(">","").replace("<","").split(".."))>=2:
				loclist.append(int(words[2].replace("join(","").replace("order(","").replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[-1]))
			else:
				loclist.append(int(words[2].replace("join(","").replace("order(","").replace("complement(","").replace(")","").replace(">","").replace("<","").split("..")[0]))
			
			loclist.sort()
		
		
		if len(words)==3 and words[1].lower() in ["snp", "misc_feature", "core", "cds", "insertion", "deletion", "recombination", "feature", "blastn_hit", "fasta_record"]  and (options.end==-1 or loclist[0]<=options.end) and (options.end==-1 or loclist[0]>=options.beginning):
			#print words[2].split("..")[0], options.beginning, len(words[2].split("..")), words[2].split("..")[1], options.end
			skip=False
			for featurename in names_to_add_feature_to:
				try: features[featurename].append((feature,colour))
				except NameError: pass #print "here"
			if words[1].lower()=="snp":
				colour="0"
			elif words[1].lower()=="misc_feature":
		...
		elif len(words)==3 and words[1].lower() in ["snp", "misc_feature", "core", "cds", "insertion", "deletion", "feature"]:
			#print "here"
			skip=True
			continue
		...
		elif options.qualifier!="" and len(words)>=2 and words[1].split("=")[0][1:]==options.qualifier:
			featurenames=''.join(words[1:]).split("=")[1].replace('"','').replace(' ','').split(',')
			y=x+1
			
			while y<len(filelines) and len(filelines[y].strip().split())>0 and filelines[y].strip().split()[1][0]!="/":
				featurenames=featurenames+''.join(filelines[y].strip().split()[1:]).replace('"','').replace(' ','').split(',')
				y+=1
			
			for featurename in featurenames:
				if not features.has_key(featurename):
					features[featurename]=[]
				if featurename in nameorder:
					names_to_add_feature_to.append(featurename)
	...
	for featurename in nameorder[::-1]:
		...
		if not featurename in features.keys():
			
			print "No features found for "+featurename
			track_number+=1
			continue
		if options.qualifier=="":
			print len(features[featurename]), "features found in", filename
		else:
			print len(features[featurename]), "features found for "+featurename+" in", filename
		
		tmpfeatures=sort_features_by_size(features[featurename])
		for x, feature in enumerate(tmpfeatures):
			#print feature
			if isinstance(feature[1], str):
				if feature[1]=="0":
					color=translator.int255_color((254,255,255))
				else:
					color = translator.artemis_color(feature[1])
			else:
				color = translator.int255_color(feature[1])
			gd_feature_set.add_feature(feature[0], color=color, label=False)
		track_number=track_number+1
	return track_number
...
#			else:
	...
	if options.circular:
		...
		#print gd_diagram.track_offsets()
		gd_diagram.draw(format="circular", circular=True, pagesize=(20*cm,20*cm), fragments=options.fragments, start=beginning, end=end)#, start=0, end=len(record))
		...
		gd_diagram.draw(format="linear", pagesize=(estimated_width*cm,21*cm), fragments=options.fragments, start=beginning, end=end, x=x, y=y)#, start=0, end=len(record))
		gd_diagram.write(options.outputfile+"_linear.pdf", "PDF")
	else:
		gd_diagram.draw(format="linear", orientation=options.orientation, pagesize=options.page, fragments=options.fragments, start=beginning, end=end)#, start=0, end=len(record))
