...
solexa_pool_m2q20.pl

=head1 SYNOPSIS

Wrapper to run Maq, extract pileup for SNPs, calculate 
95% confidence interval for the number of strains in which 
each potential SNP is present.

Uses the -m 2 -q 20 options at the assemble step.
...
$0:
 	...
 	Wrapper to run Maq, extract pileup for SNPs, calculate 
	95% confidence interval for the number of strains 
	in which each potential SNP is present.
	
	This version uses the -m 2 -q 20 options to generate
	the consensus using Maq.

Usage: $0 -i reads -r reference -n numstrains -q maxqual -p prefix -id poolid [-rep 1]
...
EOF
	...
	## run easy run
	# leave this as it's the easiest way to get the hardest part
	# done which is mapping the reads
	...
	# redo the assembly and SNP calling steps
	
	print "running maq assembly: $maq assemble -m 2 -q 20 $dir/m2q20.cns $dir/ref.bfa $dir/all.map\n";
	system `$maq assemble -N $numstrains -m 2 -q 20 $dir/m2q20.cns $dir/ref.bfa $dir/all.map`;
	
	print "maq cns2snp $dir/m2q20.cns > $dir/m2q20.cns.snp\n";
	system `maq cns2snp $dir/m2q20.cns > $dir/m2q20.cns.snp`;
	
	# filter SNP calls from non-unique regions

	$rawsnp = "$dir/m2q20.cns.snp";
    	$filtersnp = "$dir/m2q20.cns.snp.filtered";
	...
	$pileup = "$dir/m2q20.cns.snp.filtered.pileup";
	...
	while ($line=<IN>) {
		...
		if ($N ne $num_bases) {
			...
			foreach $b (1..$num_bases) {
				...
				my $weight = $qual/$maxqual; # weight of quality score
				#my $weight = (($qual-20)^2)/(($maxqual-20)^2); # weight of quality score
				#if ($qual<20) {$weight=0;}
