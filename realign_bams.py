...
def get_user_options():
	...
	group.add_option("-n", "--nodes", action="store", dest="nodes", help="Maximum number of jobs to run on nodes in parallel. [Default= %default]", default=50, type="int")
...
class bamanalysis:
	...
	def makepileup_from_sam(self, ref, bashfile):
		...
		if not os.path.isfile(self.originaldir+self.name+".bam.bai"):
			print >> bashfile, SAMTOOLS_DIR+"samtools index", self.originaldir+self.name+".bam.bai"
		
		if self.findindels:
		
#			self.keep=print_command_to_bashfile(' '.join(map(str, [BREAKDANCER_DIR+"perl/bam2cfg.pl",  self.originaldir+self.name+".bam >", self.dirname+"tmp.config"])), self.dirname, self.name, bashfile, self.log, self.keep)
#			
...
#			self.keep=print_command_to_bashfile(' '.join(map(str, [MY_SCRIPTS_DIR+"breakdancer_2_tab.py",  self.dirname+self.name+"_breakdancer.out", self.dirname+self.name+"_breakdancer.tab"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, [SAMTOOLS_DIR+"samtools view ",  self.originaldir+self.name+".bam |", PINDEL_DIR+"sam_2_pindel -", self.dirname+"tmp.pindelin 300 "+self.name+" 0"])), self.dirname, self.name, bashfile, self.log, self.keep)
...
#			self.keep=print_command_to_bashfile(' '.join(map(str, [PINDEL_DIR+"pindel -x 6 -f "+ref+" -p", self.dirname+"tmp.pindelin -c ALL -o", self.dirname+"tmp.pindel.out", "-b", self.dirname+self.name+"_breakdancer.out"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, [PINDEL_DIR+"pindel -x 6 -f "+ref+" -p", self.dirname+"tmp.pindelin -c ALL -o", self.dirname+"tmp.pindel.out"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			#NOTE: I edited this script to only output indels<2kb
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, [DINDEL_DIR+"dindel-1.01-linux-64bit --analysis getCIGARindels --bamFile", self.originaldir+self.name+".bam", "--ref", ref, "--outputFile", self.dirname+"tmp.dindel.out"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, [DINDEL_SCRIPTS_DIR+"makeWindows.py --inputVarFile", self.dirname+"tmp.dindel.out.variants_min2.txt", "--windowFilePrefix", self.dirname+"tmp.dindel.windows", "--numWindowsPerFile 1000"])), self.dirname, self.name, bashfile, self.log, self.keep)
			

			self.keep=print_command_to_bashfile(' '.join(map(str, ["for f in", self.dirname+"tmp.dindel.windows.*.txt", "; do  g=${f/windows/output};", DINDEL_DIR+"dindel-1.01-linux-64bit --analysis indels --doDiploid --bamFile", self.originaldir+self.name+".bam", "--ref", ref, "--outputFile", "${g%.txt}", "--outputRealignedBAM --processRealignedBAM", MY_SCRIPTS_DIR+"dindeltobam.sh --varFile", "$f", "--libFile", self.dirname+"tmp.dindel.out.libraries.txt", "${f}_stage2", ";done"])), self.dirname, self.name, bashfile, self.log, self.keep)
			
			
			self.keep=print_command_to_bashfile(' '.join(map(str, ["ls", self.dirname+"*.glf.txt >", self.dirname+"tmp.dindelfiles"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, ["cat", self.dirname+"tmp.dindel.output*_realigned.sam >", self.dirname+"tmp.dindel.output_realigned.sam"])), self.dirname, self.name, bashfile, self.log, self.keep)
			...
			self.keep=print_command_to_bashfile(' '.join(map(str, [MY_SCRIPTS_DIR+"realign_dindel_bams.py", self.dirname+self.name+"_dindel.bam", self.dirname+self.name+"_dindel.vcf", self.originaldir+self.name+".bam", self.dirname+"tmp.realigned.bam"])), self.dirname, self.name, bashfile, self.log, self.keep)
	...
	if options.LSF==True:
		if count>0:
			if options.mem>0:
				...
				os.system('echo \'bash ${LSB_JOBINDEX}'+tmpname+'_sbs.sh\' | bsub -R \'select[mem>'+memmb+'] rusage[mem='+memmb+']\' -q '+options.LSFQ+'  -J'+tmpname+'_'+'"[1-'+str(count)+']%'+str(options.nodes)+'" -M '+memkb+' -o '+tmpname+'-%I.out -e '+tmpname+'-%I.err')# run all ssaha jobs in job array
			else:
				os.system('echo \'bash ${LSB_JOBINDEX}'+tmpname+'_sbs.sh\' | bsub -q '+options.LSFQ+' -J'+tmpname+'_'+'"[1-'+str(count)+']%'+str(options.nodes)+'" -o '+tmpname+'-%I.out -e '+tmpname+'-%I.err')
