...
#	group.add_option("-p", "--pseudosequence", action="store", dest="pseudosequence", help="output pseudosequence including indels [default is to write a psedoalignment to the reference and separate indel text file]", default=False)
	...
	group.add_option("-A", "--ambiguities", action="store_true", dest="use_ambiguities", help="include ambiguity codes in output for heterozygous sites", default=False)
...
def get_ambiguity_code(bases):
	
	bases.sort()
	bases_string=''.join(bases).upper()

	ambiguity_codes={'A': 'A', 'C': 'C', 'G': 'G', 'T': 'T', 'GT': 'K', 'AC': 'M', 'AG': 'R', 'CT': 'Y', 'CG': 'S', 'AT': 'W', 'CGT': 'B', 'ACG': 'V', 'ACT': 'H', 'AGT': 'D', 'N': 'N', 'X': 'N'}
	
	ambiguity_code=ambiguity_codes[bases_string]
	
	return ambiguity_code
...
if __name__ == "__main__":
	...
	for line in bcffile:
		...
		if not SNP and BASEINFO["INFO"]["DP4ratios"]["fref"]<options.ratio:
			if options.use_ambiguities:
				if len(BASEINFO["ALT"].split(",")[0])==1:
					if BASEINFO["ALT"].split(",")[0]==".":
						print BASEINFO
					ambiguity=get_ambiguity_code([BASEINFO["ALT"].split(",")[0], BASEINFO["REF"]])
					print ambiguity
			keep=False
			fratiofail+=1
