	...
	for record in alignment:
		...
		for x, refbase in enumerate(refseq):
			...
			if base in unknowns:
				if ininsertion:
					ininsertion=False
					outlist.append("\t".join(map(str,[ref, insertionstart[0], "99", refseq[insertionstart[1]], sequence[insertionstart[1]]+''.join(insertion), ".", "PASS", "INDEL", "GT", "1/1"])))
					#print insertion
				elif indeletion:
					indeletion=False
					outlist.append("\t".join(map(str,[ref, deletionstart[0], "99", refseq[deletionstart[1]]+''.join(deletion), sequence[deletionstart[1]], ".", "PASS", "INDEL", "GT", "1/1"])))
				...
				if ininsertion:
					...
					inmissing=False
					outlist.append("\t".join(map(str,[ref, missingstart[0], "0", refseq[missingstart[1]]+''.join(missing), sequence[missingstart[1]], ".", "u", "INDEL", "GT", "1/1"])))
				elif base not in gaps:
					outlist.append("\t".join(map(str,[ref, refx, ".", refbase, '.', "99", "PASS", ".", "GT", '0/0'])))				
				...
				if ininsertion:
					...
					inmissing=False
					outlist.append("\t".join(map(str,[ref, missingstart[0], "0", refseq[missingstart[1]]+''.join(missing), sequence[missingstart[1]], ".", "u", "INDEL", "GT", "1/1"])))
				outlist.append("\t".join(map(str,[ref, refx, ".", refbase, base, "99", "PASS", ".", "GT", '1/1'])))
			
			elif base in gaps and refbase not in gaps:
				if ininsertion:
					ininsertion=False
					outlist.append("\t".join(map(str,[ref, insertionstart[0], ".", refseq[insertionstart[1]], sequence[insertionstart[1]]+''.join(insertion), "99", "PASS", "INDEL", "GT", "1/1"])))
				elif inmissing:
					inmissing=False
					outlist.append("\t".join(map(str,[ref, missingstart[0], "0", refseq[missingstart[1]]+''.join(missing), sequence[missingstart[1]], ".", "u", "INDEL", "GT", "1/1"])))
					#print insertion
				...
				if indeletion:
					indeletion=False
					outlist.append("\t".join(map(str,[ref, deletionstart[0], ".", refseq[deletionstart[1]]+''.join(deletion), sequence[deletionstart[1]], "99", "PASS", "INDEL", "GT", "1"])))
				elif inmissing:
					inmissing=False
					outlist.append("\t".join(map(str,[ref, missingstart[0], "0", refseq[missingstart[1]]+''.join(missing), sequence[missingstart[1]], ".", "u", "INDEL", "GT", "1"])))
					#print deletion
				if ininsertion:
		...
		if len(outlist)>0:
			...
			print >> output, '##INFO=<ID=INDEL,Number=0,Type=Flag,Description="Indicates that the variant is an INDEL.">'
			print >> output, '##FORMAT=<ID=GT,Number=1,Type=String,Description="Genotype">'
			print >> output, "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t"+record.id
			for line in outlist:
				print >>output, line
			output.close()
		else:
