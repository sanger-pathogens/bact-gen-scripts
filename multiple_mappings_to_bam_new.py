...
SEQTK_DIR="/software/pathogen/external/bin/"
...
def get_user_options():
	...
	group.add_option("-z", "--nomapid", action="store", dest="nomapid", help="Minimum identity threshold to report a mapping Specified as a positive integer or proportion of read length (smalt only) [default= %default]", default=0.9, type="float", metavar="float")
	...
	group.add_option("-N", "--refname", action="store_true", dest="refname", help="Include reference name in directory names", default=False)
...
#		
	...
	def makepileup_from_sam(self, ref, bashfile):
		...
		print >> bashfile, "rm", self.runname+"/tmp*.bam"	
		...
		if self.pairedend:
			print >> bashfile, "rm -f", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
		else:
			print >> bashfile, "rm -f", self.fastqdir+self.name+".fastq"
		#print >> bashfile, "grep -B1 Exited", tmpname+options.program+"-"+str(count)+".err >", self.runname+"/"+self.name+"_lsferrors.txt"
	...
	for pool in args:
		...
		if pool.split('.')[-1]=="gz" and pool.split('.')[-2]=="fastq":
			...
			if not os.path.isdir(tmpname+"_unzipped"):
				os.system("mkdir "+tmpname+"_unzipped")
			if options.pairedend:
				if pool.split('.')[-2][-2:] in ["_1", "_2"]:
					if not '.'.join(pool.split('/')[-1].split('.')[:-1])[:-2] in unziplist:
						unziplist['.'.join(pool.split('/')[-1].split('.')[:-1])[:-2]]=[pool]
					else:
						unziplist['.'.join(pool.split('/')[-1].split('.')[:-1])[:-2]].append(pool)
				else:
					unziplist['.'.join(pool.split('/')[-1].split('.')[:-1])]=[pool]
			else:
				unziplist['.'.join(pool.split('/')[-1].split('.')[:-1])]=[pool]
			pool=tmpname+"_unzipped/"+pool.split('/')[-1]
...
#			continue
		...
		if options.refname:
			pool=pool+"_"+options.ref.split("/")[-1].split('.')[0]+options.program
		else:
...
#				SeqIO.write([my_seq_record], open(options.output+".aln","w"), "fasta")
	...
	for pool in pools:
		...
		if options.keep and os.path.isfile(pool.runname+"/"+pool.name+".bam") and os.path.isfile(pool.runname+"/"+pool.name+".bcf") and os.path.isfile(pool.runname+"/"+pool.name+".mfa"):# and os.path.isfile(pool.runname+"/"+pool.name+"_indels.txt"):
			...
			if pool.name in ziplist:
				for filename in ziplist[pool.name]:
					print >> bashfile, SEQTK_DIR+"seqtk trimfq "+filename+" > "+pool.fastqdir+'.'.join(filename.split('/')[-1].split('.')[:-1])
			elif pool.name in unziplist:
				for filename in unziplist[pool.name]:
					print >> bashfile, SEQTK_DIR+"seqtk trimfq "+filename+" > "+pool.fastqdir+filename.split('/')[-1]
	...
	if options.LSF==True:
		...
		if options.pseudosequence:
			if len(pools)>200:
				...
				os.system('echo '+summarystring+' | bsub -M 16000000 -q long -R \'select[mem>16000] rusage[mem=16000]\' -w \'ended('+tmpname+'_joining)\' -o '+options.output+'_sum.out -e '+options.output+'_sum.err')
		#"grep -B1 Exited", tmpname+options.program+"-"+str(count)+".err >", self.runname+"/"+self.name+"_lsferrors.txt"
