...
def main():
	...
	parser.add_option("-d", "--directory", action="store", dest="Josie", help="directory containing output from Josie's script", default="")
...
#		DoError('No reference selected! If you give an embl file you must specify which sequence it is linked to (i.e. the reference)')
	...
	if options.Josie=='':
		DoError('No directory of Josie\'s results selected!')
	elif not os.path.isdir(options.Josie):
		DoError('Cannot find directory '+options.Josie+'!')
...
if __name__ == "__main__":
	...
	#read Josie's output files
	hetpositions={}
	for file in os.listdir(options.Josie):
		for line in open(options.Josie+"/"+file, "rU"):
			...
			if len(words)!=5:
				...
				try:
					...
					continue
				try:
					DP4=map(int,words[1].split(","))
				except StandardError:
					continue
				try:
					ATCG=map(int,words[2].split(","))
				except StandardError:
					continue
				try:
					freq=float(words[3])
					...
					continue
			if not words[4].rstrip("_variant") in hetpositions:
				hetpositions[words[4].rstrip("_variant")]={}
			hetpositions[words[4].rstrip("_variant")][pos]=[DP4, ATCG, freq]
		
	
	
	print len(hetpositions)
...
#		print x
		for taxonb in seqnames[x+1:]:
			...
			if len(taxonseq)==len(taxonbseq):
				...
				for y, base in enumerate(taxonseq):
					...
					if base!=taxonbseq[y] and base not in gap_and_missing and taxonbseq[y] not in gap_and_missing:
						...
						if taxonbseq[y] in gap_and_missing:
							ncount+=1
						elif taxon in hetpositions and y+1 in hetpositions[taxon]:
							ATCG=[]
							if hetpositions[taxon][1][0]>0:
								ATCG.append("A")
							if hetpositions[taxon][1][1]>0:
								ATCG.append("T")
							if hetpositions[taxon][1][2]>0:
								ATCG.append("C")
							if hetpositions[taxon][1][3]>0:
								ATCG.append("G")
							if taxonbseq[y] not in gap_and_missing and taxonbseq[y] not in ATCG:
								count+=1
							else:
								hetmatches+=1
						else:
			...
			tot+=1
			if hetmatches>0:
				print taxon, taxonb, count, ncount, hetmatches
