...
print '\t'.join(["File", "Total length", "No. contigs", "Mean length", "Stdev lengths", "Max length", "Min length", "N50", "N50n", "MeanGC", "GC% stdev", "Median GC%", "Max GC%", "MinGC%"])
...
#	print "Minimum length =", numpy.min(lengths)
	...
	while sum<fifty:
		...
		count+=1
	try:
		print '\t'.join(map(str,[filename, numpy.sum(lengths), len(lengths), numpy.mean(lengths), numpy.std(lengths), numpy.max(lengths), numpy.min(lengths), N50, count, numpy.mean(GCs), numpy.mean(GCs), numpy.mean(GCs), numpy.max(GCs), numpy.min(GCs)]))
	except StandardError:
