...
def get_user_options():
	...
	group.add_option("-j", "--mininsert", action="store", dest="mininsertsize", help="minimum insert size (ssaha and smalt only) [default= %default]", default=100, type="int", metavar="INT")
...
#		
	...
	def runSMALT(self, ref, bashfile):	
		...
		#Map the reads against the genome
		if options.pairedend:
			print >> bashfile, SMALT_DIR+" map -x -i", options.maxinsertsize, " -j", options.mininsertsize, " -f samsoft -o "+self.runname+"/tmp1.sam", tmpname+".index", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
		else:
			print >> bashfile, SMALT_DIR+" map -x -f samsoft -o "+self.runname+"/tmp1.sam", tmpname+".index", self.fastqdir+self.name+".fastq"
...
#		
	...
	def makepileup_from_sam(self, ref, bashfile):
		...
		#order the bam file
		print >> bashfile, SAMTOOLS_DIR+"samtools sort ", self.runname+"/tmp.bam", self.runname+"/"+self.name
...
#			continue
		...
		if options.pairedend==True and filetype!=".bam":
			if pool[-2:]=='_1':
				pool=pool[:-2]
				if not os.path.isfile(originalfastqdir+pool+"_2.fastq") and not os.path.isfile(originalfastqdir+pool+"_2.fastq.gz"):
					print "File "+pool+"_2.fastq not found! Skipping..."
					continue
			elif pool[-2:]=='_2':
				pool=pool[:-2]
				if not os.path.isfile(originalfastqdir+pool+"_1.fastq") and not os.path.isfile(originalfastqdir+pool+"_1.fastq.gz"):
	...
	if options.LSF==True:
		...
		if options.pseudosequence:
			if len(pools)>200:
				...
				if count==0:
					os.system('echo '+joinstring+' | bsub -J'+tmpname+'_joining -o '+options.output+'_join.out -e '+options.output+'_join.err')
				else:
					os.system('echo '+joinstring+' | bsub -J'+tmpname+'_joining -w \'ended('+tmpname+'_'+options.program+')\' -o '+options.output+'_join.out -e '+options.output+'_join.err ')
				os.system('echo '+summarystring+' | bsub -M 16000000 -q long -R \'select[mem>16000] rusage[mem=16000]\' -w \'ended('+tmpname+'_joining)\' -o '+options.output+'_sum.out -e '+options.output+'_sum.err')
