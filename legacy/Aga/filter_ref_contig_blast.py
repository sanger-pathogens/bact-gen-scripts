...
def main():
	...
	group.add_option("-i", "--minid", action="store", dest="minid", help="Minimum identity threshold for a blast match", default=0, type="float", metavar="float")
		...
		if words[1]!=lastsubject or words[0]!=lastquery:
			if lastsubject and lastquery:
				...
				if max_run>longest_run:
					longest_run=max_run
				if (not keep or lengths[lastquery]<options.minsize) and lastquery in strain_seqs:	
