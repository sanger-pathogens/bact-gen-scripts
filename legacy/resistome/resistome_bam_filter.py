...
def main():
	...
	parser.add_option("-H", "--hmm", action="store", dest="hmm", help="hmm file containing regions to treat as unmapped when using the resistome option", default="")
...
def check_input_validity(options, args):
	...
	if options.hmm!="" and not os.path.isfile(options.hmm):
		DoError('Cannot find file '+options.hmm)
...
if __name__ == "__main__":
	...
	if options.hmm!="":
		try:
			hmmlines=open(options.hmm,"rU").readlines()
		except StandardError:
			print "Failed to read tab file"
			sys.exit()
		ftloc=-1
		note=""
		for line in hmmlines:
		
			line=line.strip()
			words=line.split()
			
			if len(line)==0 or line[0]=="#" or len(words)!=23:
				continue
			
			contig=words[0]
			contigstart=int(words[17])
			contigend=int(words[18])
			
			print contig, contigstart, contigend
			
			if not contig in regions:
				regions[contig]=[]
			regions[contig].append([contigstart, contigend])
...
#		contigstart=contigend
	...
	for contig in regions:
		for region in xrange(len(regions[contig])):
			for read in samfile.fetch(contig, regions[contig][region][0], regions[contig][region][1]):
				reads_to_remove.add(read.qname)
	
	print len(reads_to_remove), "reads to be removed in hmm hits"
	...
	for read in samfile:
		...
		elif options.outputtype=="atleastonemapped" and (not read.is_unmapped or not read.mate_is_unmapped):
			if options.fileformat=="pairedfastq":
				if readname in firstofpair:
					printread=True
				else:
		...
		elif (read.is_unmapped and read.mate_is_unmapped) and (options.outputtype=="bothunmapped" or options.outputtype=="resistome" or options.outputtype=="atleastoneunmapped"):
			if options.fileformat=="pairedfastq":
				if readname in firstofpair:
					printread=True
				else:
		...
		elif (read.is_unmapped or read.mate_is_unmapped) and (options.outputtype=="oneunmapped" or options.outputtype=="resistome" or options.outputtype=="atleastoneunmapped"):
			if options.fileformat=="pairedfastq":
				if readname in firstofpair:
					printread=True
				else:
		...
		elif options.outputtype=="resistome" and read.qname in reads_to_remove:
			if options.fileformat=="pairedfastq":
				if readname in firstofpair:
					printread=True
				else:
