...
#	os.system("export EUSRC=/nfs/users/nfs_s/sh16/euler-sr-ec-2.0.2")
	...
	returnval=os.system("mv fixed/"+tmpname+".fasta "+tmpname+".fasta")
	if returnval!=0:
		print "mv failed"
		sys.exit()
	...
	linecount=bufcount(tmpname+"_1.fasta")
...
#			os.system(velvet_dir+"velvetg "+tmpname+"_velvet -ins_length 200 -unused_reads yes -long_mult_cutoff 0 -exp_cov "+str(expectedcoverage)+" -cov_cutoff "+str(0.5*expectedcoverage))
#		else:
#			os.system(velvet_dir+"velveth "+tmpname+"_velvet 21,61,10 -"+filetype+" -shortPaired "+tmpname+"_"+str(x+1)+"."+filetype)
#			os.system(velvet_dir+"velvetg "+tmpname+"_velvet -ins_length 200 -unused_reads yes -exp_cov "+str(expectedcoverage)+" -cov_cutoff "+str(0.5*expectedcoverage))
		
		os.system(velvet_dir+"velveth "+tmpname+"_velvet 21,61,10 -"+filetype+" -shortPaired "+tmpname+"_"+str(x+1)+"."+filetype)
		os.system(velvet_dir+"velvetg "+tmpname+"_velvet -ins_length 200 -min_contig_lgth 100 -scaffolding no -unused_reads yes -exp_cov "+str(expectedcoverage)+" -cov_cutoff "+str(0.5*expectedcoverage))
		
		if os.path.isfile(tmpname+"_velvet/contigs.fa"):
			os.system("cat "+tmpname+"_velvet/contigs.fa >> "+tmpname+"contigs")
#			os.system("mv "+tmpname+"_velvet/contigs.fa "+tmpname+"contigs")
			os.system("cat "+tmpname+"_velvet/UnmappableReads.fa >> "+tmpname+"unmapped")
		else:
	...
	for i in range(count):
		unmappedexpectedcoverage=(expectedcoverage/linecount)*bufcount(tmpname+"unmapped")
		os.system(velvet_dir+"velveth "+tmpname+"_velvet 21,61,10 -"+filetype+" -shortPaired "+tmpname+"unmapped")
		os.system(velvet_dir+"velvetg "+tmpname+"_velvet -ins_length 200 -min_contig_lgth 100 -unused_reads yes -scaffolding no -unused_reads yes -exp_cov "+str(unmappedexpectedcoverage)+" -cov_cutoff "+str(0.5*unmappedexpectedcoverage))
		os.system("cat "+tmpname+"_velvet/contigs.fa >> "+tmpname+"contigs")
		os.system("cat "+tmpname+"_velvet/UnmappableReads.fa > "+tmpname+"unmapped")
		os.system("rm -rf "+tmpname+"_velvet")
	os.system(velvet_dir+"velveth "+tmpname+"_velvet 21,61,10 -"+filetype+" -long "+tmpname+"contigs")
	os.system(velvet_dir+"velvetg "+tmpname+"_velvet -min_contig_lgth 100 -scaffolding no")
	
	

	os.system("mv "+tmpname+"_velvet/contigs.fa "+options.outputname)
