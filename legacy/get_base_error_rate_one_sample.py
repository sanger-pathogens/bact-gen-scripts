...
def check_input_validity(options, args):
	...
	if options.output=='':
		options.output=options.bcf.split("/")[-1].split(".")[0]+"_het_data.txt"
...
#		DoError('Cannot find file '+options.bam)
	if options.bcf=='':
		DoError('No bcf file (-b) selected!')
	...
	for line in bcffile:
		...
		if "DP4" in BASEINFO['INFO']:
			if (BASEINFO['INFO']["DP4"][0]+BASEINFO['INFO']["DP4"][1])>(BASEINFO['INFO']["DP4"][2]+BASEINFO['INFO']["DP4"][3]):
				try:
					refdata[BASEINFO["CHROM"]][int(BASEINFO["POS"])-1].append((BASEINFO['INFO']["DP4"][2]+BASEINFO['INFO']["DP4"][3])/(BASEINFO['INFO']["DP4"][0]+BASEINFO['INFO']["DP4"][1]))
				except IndexError:
					print BASEINFO["POS"], len(refdata[BASEINFO["CHROM"]])
			else:
				try:
					refdata[BASEINFO["CHROM"]][int(BASEINFO["POS"])-1].append((BASEINFO['INFO']["DP4"][0]+BASEINFO['INFO']["DP4"][1])/(BASEINFO['INFO']["DP4"][2]+BASEINFO['INFO']["DP4"][3]))
				except IndexError:
					print BASEINFO["POS"], len(refdata[BASEINFO["CHROM"]])
		else:
...
#			print lastbase, int(BASEINFO["POS"])
		x=lastbase
		while x<int(BASEINFO["POS"]):
			refdata[BASEINFO["CHROM"]][x-1].append(0)
			x+=1
