...
#		bobnode=tnode
#		boblen=0.0
#		while bobnode is not None and bobnode is not treeobject.root:
...
def copy_branchlengths(intree, outtree):
			
	def copy_branchlength(nodenum):
		outtree.node(nodenum).data.branchlength=intree.node(nodenum).data.branchlength
		
		daughters=intree.node(nodenum).succ
		for daughter in daughters:
			copy_branchlength(daughter)
		
	copy_branchlength(intree.root)
	return outtree
...
#		total_terminal_lengths={}
#		for tterminal in tree.get_terminals():
#			total_terminal_lengths[tterminal]=tree.sum_branchlength(tree.root,tterminal)
#		print total_terminal_lengths
#		tree.display()
#		print oldtree
#		print newtree
		...
		pamltreefile2=os.popen('grep -a -A 2 "Ancestral reconstruction by BASEML." rstnew | tail -n 1')
		
		pamltreestring2=pamltreefile2.read().strip()
#		print pamltreestring
#		print pamltreestring2
...
#		pamltree.display()
		...
		pamltree2=Trees.Tree(pamltreestring2, rooted=True)
		
		pamltree=copy_branchlengths(pamltree2, pamltree)
#		pamltree2.display()
#		pamltree.display()
...
#	os.system("mv "+prefix+"_iteration"+str(iteration)+".tre "+prefix+"_Final.tre")
	if options.reference=="":
		os.system("/nfs/pathogen/sh16_scripts/reportlabtest.py -a 2 -q taxa -M -L right -t "+prefix+"_Final.tre -o "+prefix+"_Final_recomb.pdf "+prefix+"_rec.tab")
	else:
		os.system("/nfs/pathogen/sh16_scripts/reportlabtest.py -q taxa -a 2 -M -L right -t "+prefix+"_Final.tre -o "+prefix+"_Final_recomb.pdf "+options.reference+" "+prefix+"_rec.tab")
