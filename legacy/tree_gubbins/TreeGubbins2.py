...
def read_tree(treefile, verbose=False):
	...
	for treeschema in ["nexus", "newick"]:#["beast-summary-tree",  "nexus", "newick"]:
		try: 
			...
			if verbose:
				print "Encountered DataParseError while trying to read tree file as", treeschema
			continue
	...
	if not opened:
		...
		print "Tree file opened successfully"
...
def get_tree_node_likelihoods(t, treelength, node_count, multiplier, verbose=False):
	...
	for leaf in tree.leaf_node_iter():
		treelength-=leaf.edge_length
		leaf_count+=1
	
	for node in t.postorder_internal_node_iter():
	
		if node.downstream_count<3 or node==tree.seed_node:
			continue
		
		lenleaf=0
		for leaf in node.leaf_iter():
			node.downstream_length-=leaf.edge_length
			lenleaf+=1
		if lenleaf>(leaf_count-3):
			continue
		
		#assertAlmostEqual(lenleaf, len(node.downstream_set), places=7, message=len(node.downstream_set))# len(node.downstream_set)
		...
		#assertAlmostEqual(node.upstream_length+node.downstream_length, treelength, places=7, message=node.upstream_length+node.downstream_length)
		#assert (node.upstream_length+node.downstream_length)==treelength, "%f" % (node.upstream_length+node.downstream_length)
		
		
		if (node.downstream_count)>2 and (node.upstream_count+1)>0 and (multiplier*treelength)/node_count>((multiplier)*node.downstream_length)/node.downstream_count:
		...
		if (node.upstream_count)>2 and (node.downstream_count+1)>0 and (multiplier*treelength)/node_count>((multiplier)*node.upstream_length)/node.upstream_count:
...
if __name__ == "__main__":
	...
	try:
		original_tree=read_tree(options.tree, verbose=options.verbose)
	...
	while significant:
		...
		likelihoods=get_tree_node_likelihoods(tree, treelength, node_count, multiplier, verbose=options.verbose)
