...
def detect_recombination_using_moving_windows(binsnps, treeobject, node, daughter, nodenames, daughternames, downstreamtaxa):
	...
	for block in blocks:
		...
		if node==0 and options.outgroup!="" and options.outgroup!="None":
			...
			rgbcolor=' '.join(map(str,map(int,treeobject.node(daughter).data.comment["colour"])))
			if treeobject.is_internal(daughter):
				print >> tabout, "FT                   /colour="+rgbcolor
				print >> tabout, 'FT                   /taxa="'+', '.join(downstreamnamelist)+'"'
				print >> tabout, 'FT                   /node="'+nodenames[1]+'->'+daughternames[daughter][1]+'"'
...
def create_comment_dictionaries_on_nodes(treeObject):
	
	def create_comment_dictionaries_on_node(node):
		
		daughters=treeObject.node(node).succ
		for daughter in daughters:
			 create_comment_dictionaries_on_node(daughter)
		treeObject.node(node).data.comment={}
	
	create_comment_dictionaries_on_node(treeObject.root)
	return treeObject
	

def copy_comments_across_tree(treeobjectfrom, treeobjectto):
	def copy_node_comments(node):
		
		daughters=treeobjectto.node(node).succ
		for daughter in daughters:
			 copy_node_comments(daughter)
		treeobjectto.node(node).data.comment=copy.deepcopy(treeobjectfrom.node(node).data.comment)
	
	copy_node_comments(treeobjectto.root)
	return treeobjectto
	
def colour_nodes_by_depth(treeobject):
	...
	def get_max_branch_depth():
		...
		for terminal in terminals:
			brlen=treeobject.sum_branchlength(node=terminal)-(float(treeobject.node(terminal).data.branchlength)/2)
			if brlen>maxbrlen:
				maxbrlen=brlen
		minbrlen=float("Inf")
		for internal in treeobject.node(treeobject.root).succ:
			brlen=treeobject.sum_branchlength(node=internal)-(float(treeobject.node(internal).data.branchlength)/2)
			if brlen<minbrlen:
				minbrlen=brlen
		
		return maxbrlen, minbrlen
	
	def get_rgb_colour(depth, maxbrlen, minbrlen):

		
#		proportion=((float(depth)-minbrlen)/(maxbrlen-minbrlen))*510
#		print depth, minbrlen, maxbrlen, float(depth)-minbrlen, maxbrlen-minbrlen, proportion
#		
#		red=510-proportion
#		blue=proportion-255
#		green=proportion
...
#		return (red, green, blue)
		...
		proportion=((float(depth)-minbrlen)/(maxbrlen-minbrlen))*255
#		print depth, minbrlen, maxbrlen, float(depth)-minbrlen, maxbrlen-minbrlen, proportion
...
#		proportion=((float(depth)-minbrlen)/(maxbrlen-minbrlen))*1275
#		red=510-proportion
#		blue=proportion-510
#		green=proportion
#		if red<-510:
...
#				green=0
	...
	def colour_node_by_depth(node):
		
		daughters=treeobject.node(node).succ
		for daughter in daughters:
			 colour_node_by_depth(daughter)
		if treeobject.node(node).data.comment==False:
			treeobject.node(node).data.comment={}
		if node==treeobject.root:
			treeobject.node(node).data.comment["colour"]=(0,0,0)
		else:
			treeobject.node(node).data.comment["colour"]=get_rgb_colour(treeobject.sum_branchlength(node=node)-(float(treeobject.node(node).data.branchlength)/2), maxbrlen, minbrlen)
		
	maxbrlen, minbrlen=get_max_branch_depth()
	colour_node_by_depth(treeobject.root)
#	sys.exit()
	return treeobject
...
#				os.system("rm tmptrees.tre rfout.tmp")
		...
		if (iteration>1 and rf==0) or oldtree.is_identical(newtree):
			break
		...
		try:
			tree_string = open(prefix+".tre").read()
				
		except IOError:
			DoError("Cannot open tree file "+prefix+".tre")
		printtree = Trees.Tree(tree_string, rooted=True)
		printtree=create_comment_dictionaries_on_nodes(printtree)
		printtree=colour_nodes_by_depth(printtree)
		
		#copy the branch colours onto the paml tree
		pamltree=copy_comments_across_tree(printtree, pamltree)
...
#			os.system("rm tmptrees.tre gdout.tmp rfout.tmp")
	...
	#print the final tree with branch colours as comments
	
	terminals=printtree.get_terminals()
	for terminal in terminals:
		if printtree.node(terminal).data.taxon in sequencenames:
			printtree.node(terminal).data.taxon=sequencenames[printtree.node(terminal).data.taxon]
	treestring=tree_to_string(printtree, False, True, True, True,collapse=False, cutoff=1.0/(len(SNPlocations)+1), comments=True)
	...
	if options.reference=="":
		os.system("/nfs/pathogen/sh16_scripts/reportlabtest.py -q taxa -t "+prefix+"_Final.tre -o "+prefix+"_Final_recomb.pdf "+prefix+"_rec.tab")
	else:
		os.system("/nfs/pathogen/sh16_scripts/reportlabtest.py -q taxa -t "+prefix+"_Final.tre -o "+prefix+"_Final_recomb.pdf "+options.reference+" "+prefix+"_rec.tab")
