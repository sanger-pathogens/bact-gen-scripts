...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
	...
	if ($last_chr ne $t[0]) {
	  ...
	  &v2q_post_process($_o, $last_chr, \$seq, \$qual, \@gaps, $opts{l}) if ($last_chr);
  ...
  $_o = $last_chr if ($_o eq 'sequence');
  &v2q_post_process($_o, $last_chr, \$seq, \$qual, \@gaps, $opts{l});
}

sub v2q_post_process {
  my ($seqname, $chr, $seq, $qual, $gaps, $l) = @_;
  for my $g (@$gaps) {
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
	...
	if ($last_chr ne $t[0]) {
	  $_o = $last_chr if ($_o eq 'sequence');
	  &v2a_post_process($_o, $last_chr, \$seq, \$qual, \@gaps, $opts{l}) if ($last_chr);
	  ($last_chr, $last_pos) = ($t[0], 0);
	  $seq = $qual = '';
	  @gaps = ();
	}
  ...
  $_o = $last_chr if ($_o eq 'sequence');
  &v2a_post_process($_o, $last_chr, \$seq, \$qual, \@gaps, $opts{l});
}
