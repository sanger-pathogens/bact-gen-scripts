...
if __name__ == "__main__":
	...
	for contig in contigorder:
		majcontigs[contig]=["N"]*contigsizes[contig]
		mincontigs[contig]=["N"]*contigsizes[contig]
	...
	for line in bcffile:
		...
		if words[0][0]=="#":
			if words[0][1]!="#":
				...
				if options.qratio>0:
					print >> vcf, "##FILTER=<ID=QRR"+str(options.qratio)+', Description="Ratio of high quality calls to total calls below '+str(options.qratio)+' at non-variant site"'
					print >> vcf, "##FILTER=<ID=QRA"+str(options.qratio)+', Description="Ratio of high quality calls to total calls below '+str(options.qratio)+' at variant site"'
				if options.AF1>0:
		...
		if not "DP4" in BASEINFO["INFO"]:
			...
			try: BASEINFO["INFO"]["DP4ratios"]["ralt"]=float(BASEINFO["INFO"]["DP4"][3])/(BASEINFO["INFO"]["DP4"][1]+BASEINFO["INFO"]["DP4"][3])
				...
				BASEINFO["INFO"]["DP4ratios"]["ralt"]=0.0
			try:
				BASEINFO["INFO"]["DP4ratio"]=(float(BASEINFO["INFO"]["DP4"][0])+float(BASEINFO["INFO"]["DP4"][1]))/(BASEINFO["INFO"]["DP4"][0]+BASEINFO["INFO"]["DP4"][1]+BASEINFO["INFO"]["DP4"][2]+BASEINFO["INFO"]["DP4"][3])
			except ZeroDivisionError:
				BASEINFO["INFO"]["DP4ratio"]=0.0
...
#			failedfilters.append("QRR"+str(options.qratio))
		...
		if not SNP and BASEINFO["INFO"]["DP4ratio"]<options.ratio:
			ratfail=True
...
#			failedfilters.append("AF"+str(options.AF1))
		...
		if keep:
			...
			if SNP:
				...
				snps+=1
				if INDEL and majcontigs[BASEINFO["CHROM"]][int(BASEINFO["POS"])-1]!="N": #if it's an indel check the previous base has been mapped
...
#				sys.exit()
	...
	out=open(options.output+"_maj.mfa","w")
	
	name=options.output.split("/")[-1].split('.')[0]
	
	for x, contig in enumerate(contigorder):
		print >> out, ">"+name+"_maj_"+''.join(contig)
		print >> out, ''.join(majcontigs[contig])
	out.close()
	
	out=open(options.output+"_min.mfa","w")
	
	name=options.output.split("/")[-1].split('.')[0]
	
	for x, contig in enumerate(contigorder):
		print >> out, ">"+name+"_min_"+''.join(contig)
		print >> out, ''.join(mincontigs[contig])
	out.close()
