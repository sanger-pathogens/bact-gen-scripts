...
def Usage():
	...
	print "-M Do not ask for lots of memory from pcs4"
...
def getOptions(arg):
	...
	try:
		opts, args = getopt.getopt(argv, "hd:r:i:j:pq:vIm:n:P:t:R:e:sML", ["depth=", "help", "ref=", "align", "quality=", "program=", "velvet=", "interactive", "maxsnps=", "pairedend", "maxinsert=", "mininsert=", "mismatches=", "rtype=", "readlength=", "embl=", "stupid", "highmem", "LSF"])
	...
	if interactive=='y':
		ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF=menusystem(ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF)
	...
	return ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF
...
def menusystem(ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF):
	...
	if ui=='y':
		...
		while not os.path.isfile(ref):
			...
			if ref=='Q':
				ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF=menusystem(oldref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF)
	...
	def runSsaha(self, ref, tmpname):	
		...
		if pairedend=='n':
			if rtype=='454':
				...
				print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -seeds 5 -output sam_soft -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/shuffled.tmp > "+self.runname+"/cigar1.tmp"
			else:
				print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -skip 2 -diff 0 -output cigar "+ref+" "+self.fastqdir+self.name+".fastq > "+self.runname+"/cigar1.tmp"
			...
			print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_cigar "+self.runname+"/cigar1.tmp "+self.runname+"/cigar2.tmp"
			...
			if rtype=='454':
				...
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -solexa 0 -trans 0 "+self.runname+"/cigar2.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/snp.tmp"
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -cons 1 -solexa 0 -trans 0 "+self.runname+"/cigar2.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/pileup.tmp"
			elif rtype=='solexa':
				print >> bashfile, "/nfs/users/nfs_t/tdo/bin/pileup_v0.4/ssaha_pileup/other_codes/get_seqreads/get_seqreads "+self.runname+"/readnames.tmp "+self.fastqdir+self.name+".fastq "+self.runname+"/fastq.tmp"
				if velvet=='y':
					...
					print >> bashfile, "mv "+self.name+"unmap.fastq "+self.runname+"/unmap.fastq"
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -solexa 1 -trans 0 "+self.runname+"/cigar2.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/snp.tmp"
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -cons 1 -solexa 1 -trans 0 "+self.runname+"/cigar2.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/pileup.tmp"
		...
		#paired end
			...
			if rtype=='454':
				...
				#print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -seeds 5 -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/shuffled.single.tmp > "+self.runname+"/cigarsingle.tmp"
				
				#map paired reads
				print >> bashfile, "echo Mapping paired reads"
				#print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -pair "+str(self.mininsertsize)+','+str(self.maxinsertsize)+" -seeds 5 -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/shuffled.forward.tmp "+self.runname+"/shuffled.reverse.tmp > "+self.runname+"/cigarpair.tmp"
				#print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/shuffled.paired.tmp > "+self.runname+"/cigarpair.tmp"
				...
				#print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/shuffled.tmp > "+self.runname+"/cigar1.tmp"
				print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -skip 2 -diff 0 -kmer 13 -outfile "+self.runname+"/abnormal.tmp -pair "+str(self.mininsertsize)+","+str(self.maxinsertsize)+" -output cigar "+ref+" "+self.runname+"/shuffled.forward.tmp "+self.runname+"/shuffled.reverse.tmp > "+self.runname+"/cigar1.tmp"
				...
				if multiplexnamed=='y':
					...
					print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -skip 2 -diff 0 -kmer 13 -outfile "+self.runname+"/abnormal.tmp -pair "+str(self.mininsertsize)+","+str(self.maxinsertsize)+" -output cigar "+ref+" "+self.fastqdir+'_'.join(self.name.split('_')[:2])+"_1_"+self.name.split('_')[-1]+".fastq "+self.fastqdir+'_'.join(self.name.split('_')[:2])+"_2_"+self.name.split('_')[-1]+".fastq > "+self.runname+"/cigar1.tmp"
					
				else:
				
					print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -skip 2 -diff 0 -kmer 13 -outfile "+self.runname+"/abnormal.tmp -pair "+str(self.mininsertsize)+","+str(self.maxinsertsize)+" -output cigar "+ref+" "+self.fastqdir+self.name+"_1.fastq "+self.fastqdir+self.name+"_2.fastq > "+self.runname+"/cigar1.tmp"
			...
			if rtype=='solexa' and multiplexnamed=='y':
				print >> bashfile, "/nfs/pathogen/sh16_scripts/shufflefastqSequences.pl "+self.fastqdir+'_'.join(self.name.split('_')[:2])+"_1_"+self.name.split('_')[-1]+".fastq "+self.fastqdir+'_'.join(self.name.split('_')[:2])+"_2_"+self.name.split('_')[-1]+".fastq "+self.runname+"/shuffled.tmp"
			elif rtype=='solexa':
				print >> bashfile, "/nfs/pathogen/sh16_scripts/shufflefastqSequences.pl "+self.fastqdir+self.name+"_1.fastq "+self.fastqdir+self.name+"_2.fastq "+self.runname+"/shuffled.tmp"
			
			
			print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/other_codes/get_seqreads/get_seqreads "+self.runname+"/readnames.tmp "+self.runname+"/shuffled.tmp "+self.runname+"/fastq.tmp"
...
#			if pairedend=='y':
#				if rtype=='454':
#					print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -seeds 5 -score "+str(self.quality)+" -kmer 13 -skip 4 -diff 0 -output cigar "+ref+" "+self.runname+"/unmap.fastq > "+self.runname+"/unmapcigar1.tmp"
#				else:
#					print >> bashfile, SSAHA_DIR+"ssaha2 -"+self.rtype+" -score "+str(self.quality)+" -skip 2 -diff 0 -output cigar "+ref+" "+self.runname+"/unmap.fastq > "+self.runname+"/unmapcigar1.tmp"
#				print >> bashfile, 'grep "^cigar" '+self.runname+"/unmapcigar1.tmp > "+self.runname+"/unmapcigar2.tmp"
#				print >> bashfile, "mv "+self.runname+"/unmapcigar2.tmp "+self.runname+"/unmap.cigar"
			
			
			if rtype=='454':
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -solexa 0 -trans 0 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/snp.tmp"
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -cons 1 -solexa 0 -trans 0 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/pileup.tmp"
			elif rtype=='solexa':
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -solexa 1 -trans 0 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/snp.tmp"
				print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_pileup -cons 1 -solexa 1 -trans 0 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/fastq.tmp > "+self.runname+"/pileup.tmp"
			print >> bashfile, "egrep ^SNP "+self.runname+"/snp.tmp > "+self.runname+"/all.snp"
			print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_indel -insertion 1 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/pileup.tmp > "+self.runname+"/insertions.txt"
			print >> bashfile, SSAHA_DIR+"pileup_v0.6/ssaha_pileup/ssaha_pileup/ssaha_indel -deletion 1 "+self.runname+"/cigar1.tmp "+ref+" "+self.runname+"/pileup.tmp > "+self.runname+"/deletions.txt"
			print >> bashfile, "egrep ^cons "+self.runname+"/pileup.tmp > "+self.runname+"/all.pileup"
		...
		print >> bashfile, "/nfs/pathogen/sh16_scripts/cigar2plot.py "+ref+" "+self.runname+"/all.cigar "+self.runname+"/"+self.name+"_mappedcoverage.plot"
		
		if pairedend=='y':
			print >> bashfile, "/nfs/pathogen/sh16_scripts/cigar2_odd_plot.py "+ref+" "+self.runname+"/abnormal.cigar "+str(self.maxinsertsize)+" "+str(self.mininsertsize)+" 50000 "+self.runname+"/"+self.name
			print >> bashfile, "/nfs/pathogen/sh16_scripts/cigar2_fri_plot.py "+ref+" "+self.runname+"/all.cigar "+str(self.maxinsertsize)+" "+str(self.mininsertsize)+" "+self.runname+"/"+self.name+"_fricoverage.plot"
		else:
			print >> bashfile, "/nfs/pathogen/sh16_scripts/cigar2_fr_plot.py "+ref+" "+self.runname+"/all.cigar "+str(self.maxinsertsize)+" "+str(self.mininsertsize)+" "+self.runname+"/"+self.name+"_frcoverage.plot"
...
if __name__ == "__main__":
        argv=sys.argv[1:]
        ref, inputdirs, quality, velvet, mindepth, maxsnps, pairedend, maxinsertsize, mininsertsize, mismatches, program, rtype, readlength, multiplexnamed, highmem, LSF=getOptions(argv)
	...
	if program=='ssaha':
		for pool in pools:
			...
			if not LSF:
				os.system('bash '+pool.number+tmpname+'_sbs.sh')

		if LSF:
	
			if highmem=="y":
				os.system('echo \'bash ${LSB_JOBINDEX}'+tmpname+'_sbs.sh\' | bsub -R \'select[mem>10000] rusage[mem=10000]\'  -J'+tmpname+'_ssaha"[1-'+str(count)+']%16"  -M 10000000 -o '+tmpname+'ssaha-%I.out -e '+tmpname+'ssaha-%I.err')# run all ssaha jobs in job array
			else:
				...
				#os.system('echo \'bash ${LSB_JOBINDEX}'+tmpname+'_sbs.sh\' | bsub -J'+tmpname+'_ssaha"[1-'+str(count)+']%20" -M 5000000 -o '+tmpname+'ssaha-%I.out -e '+tmpname+'ssaha-%I.err')# run all ssaha jobs in job array
			
			#command='for FILENAME in '+tmpname+'*.err; do g=${FILENAME%-*}; h=${FILENAME##*-}; rm ${h%.err}${g}_sbs.sh; FILESIZE=$(stat -c%s "$FILENAME") ;echo $FILESIZE; if [ $FILESIZE -eq "0" ] ; then rm $FILENAME ${FILENAME%.err}.out; fi; done '
			#print command
			
			#os.system("bsub -w 'ended("+tmpname+"_ssaha)' "+command)
