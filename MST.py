...
def main():
	...
	parser.add_option("-c", "--colours", action="store", dest="colours", help="colour file name", default="", metavar="FILE")
	...
	parser.add_option("-T", "--transformation", action="store", dest="transformation", help="transformation type (acctran, deltran or ML). [Default= %default]", default="deltran", type="choice", choices=["acctran","deltran", "ML"])
...
#		DoError('No reference selected! If you give an embl file you must specify which sequence it is linked to (i.e. the reference)')
	...
	elif options.colours!='' and not os.path.isfile(options.colours):
		DoError('Cannot find file '+options.colours+'!')
...
#	sys.exit()
	...
	if options.colours!='' and os.path.isfile(options.colours):
		colourdict={}
		for line in open(options.colours, "rU"):
			words=line.strip().split(",")
			colourdict[words[0]]=words[1]
	...
	for x, node in enumerate(nodes):
		...
		for nodeb in nodes[x+1:]:
			if tree.node(node).get_data().comment["sequence"]==tree.node(nodeb).get_data().comment["sequence"]:
				toprint=False
				if not nodeb in sizedict:
					sizedict[nodeb]=0
				if node in sizedict:
					sizedict[nodeb]+=sizedict[node]+1
				else:
					sizedict[nodeb]+=1
		...
		if toprint:
			...
			newseq=[]
			wholeseq=tree.node(node).get_data().comment["sequence"]
			for loc in SNPlocations:
				newseq.append(wholeseq[loc])
			
			print >> outfile, str(''.join(newseq))
			if options.colours!="":
				colourlist.append(colourdict[tree.node(node).data.taxon])
			if node in sizedict:
				sizelist.append(sizedict[node]+1)
			else:
				sizelist.append(1)
	...
	if options.colours!="":
		colourout=open(options.prefix+"_colours.txt", "w")
		print >> colourout, '"'+'","'.join(colourlist)+'"'
		colourout.close()

	sizeout=open(options.prefix+"_sizes.txt", "w")
	print >> sizeout, ','.join(map(str,sizelist))
	sizeout.close()

	
	rout=open(options.prefix+"_rfile.prog", "w")

	print >> rout, "library(ape)"
	print >> rout, "library(vegan)"
	if options.colours!="":
		print >> rout, "colour.file<-read.csv(file=\""+options.prefix+"_colours.txt\",header=FALSE);"
		print >> rout, "colour.list<-as.matrix(colour.file);"
	print >> rout, "size.file<-read.csv(file=\""+options.prefix+"_sizes.txt\",header=FALSE);"
	print >> rout, "size.list<-as.matrix(size.file);"
	print >> rout, "x<-read.dna(file=\""+options.prefix+"_nogaps.aln\",format=\"fasta\")"
	print >> rout, "y<-dist.dna(x,pairwise.deletion=FALSE)"
	print >> rout, "mst<-spantree(y,toolong=0)"
	print >> rout, "mst;"
	print >> rout, "pdf(file=\""+options.prefix+"_MST.pdf\",height=10,width=10)"
	if options.colours!="":
		print >> rout, "plot(mst, col=colour.list, pch=19, cex=size.list)"
	else:
		print >> rout, "plot(mst, cex=size.list)"
	...
	redraw=True
	while redraw:
		os.system("R CMD BATCH "+options.prefix+"_rfile.prog")
