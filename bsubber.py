...
def get_user_options():
	usage = "usage: %prog [options] <script/program to bsub or full path of checkpoint directory to restart a job>"
	...
	parser.add_option("-c", "--c", action="store_true", dest="checkpoint", help="Checkpoint your job (so it can be restarted if it dies due to hardware failure). You will need to remember the job id. [Default= False]", default=False)
	...
	parser.add_option("-r", "--restart", action="store_true", dest="restart", help="Restart a checkpointed job. To do this you will need to provide the full path to the checkpoint file, e.g. /my/checkpoint/directory/<JOBID>. Important: Check that your job has really died; restarting a job from a checkpoint whilst the original is running will lead to unpredictable results! If your jobs needs special resources, you should add them to the command. (LSF does not remember them from the original bsub). ", default=False)
