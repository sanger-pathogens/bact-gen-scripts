...
if len (sys.argv)!=7 or '-h' in sys.argv[1:]:
	print "crunch_orderer.py <crunch file> <contig fasta file> <reference fasta file> <id cutoff> <score cutoff> <min length>"
	sys.exit()
...
for i, key in enumerate(keys):
	...
	if i+1<len(keys) and crdb[keys[i+1]][7]==curcontig and crtemp[7]!=curcontig:
		todelete.append(key)
		continue
	
	offset=0
	#Identify where a contig overlaps the previous one. Needs work on rev/revcomp!!!!and crtemp[7]!=curcontig
	if i>0  and int(crdb[keys[i-1]][3])>int(crtemp[2]):
		print "found overlap: ", crdb[keys[i-1]][7],crtemp[7], int(crdb[keys[i-1]][3])-int(crtemp[2]),
		#print crdb[keys[i-1]]
		#print crtemp
		if lastdirn=='f':
			lastseq= contigs[curcontig][(curend-1)-(int(crdb[keys[i-1]][3])-int(crtemp[2])):curend]
		else:
			lastseq= revcomp(contigs[curcontig][curstart-1:curstart+(int(crdb[keys[i-1]][3])-int(crtemp[2]))])
		if dirn=='f':
			newseq= contigs[crtemp[7]][int(crtemp[5])-1:int(crtemp[5])+(int(crdb[keys[i-1]][3])-int(crtemp[2]))]
		else:
			newseq= revcomp(contigs[crtemp[7]][int(crtemp[6])-(int(crdb[keys[i-1]][3])-(int(crtemp[2])-1)):int(crtemp[6])])
		if lastseq==newseq:
			
			offset=int(crdb[keys[i-1]][3])-int(crtemp[2])
			countdiffsa=0
			countdiffsb=0
			refseq=ref[int(crtemp[2])-1:int(crdb[keys[i-1]][3])]
			...
			countdiffsa=0
			countdiffsb=0
			print len(lastseq), len(newseq)
			if len(lastseq)<len(newseq):
				smallerseqlen=len(lastseq)
			else:
				smallerseqlen=len(newseq)
			for x in range(smallerseqlen):
				base=lastseq[x]
				if base!=newseq[x]:
			...
			if 100-(float(countdiffsa)/len(refseq)*100) > 100-(float(countdiffsb)/len(refseq)*100):
				todelete.append(keys[i])
			else:
				todelete.append(keys[i-1])
#			print lastseq
#			print newseq
#			print refseq
	if crtemp[7]!=curcontig or dirn!=lastdirn:
		#print crtemp, dirn, lastdirn
		if curcontig!='':
			if colour=='10':
				colour='11'
			else:
		...
		curcontig=crtemp[7]
		curstart=int(crtemp[5])
		curend=curstart
	lastdirn=dirn


	if int(crtemp[5])<curstart:
		curstart=int(crtemp[5])
	if int(crtemp[6])>curend:
		curend=int(crtemp[6])

	
	curcrunch[int(crtemp[5])]=crtemp[:]


for entry in todelete:
	if crdb.has_key(entry):
		del crdb[entry]
