...
import string, re
import os, sys, getopt, math, time, datetime
from random import *
...
def get_user_options():
	...
	group.add_option("-5", "--nomap", action="store_false", dest="domapping", help="Do not remap data - only available when input is bam (default is to map)", default=True)
	...
	group.add_option("-1", "--matchscore", action="store", dest="match_score", help="Score for a match in Smith-Waterman alignment [default= %default]", default=1, type="int")
	group.add_option("-2", "--substscore", action="store", dest="subst_score", help="Score for a substitution in Smith-Waterman alignment [default= %default]", default=-2, type="int")
	group.add_option("-3", "--gapopenscore", action="store", dest="gap_open_score", help="Score for opening a gap in Smith-Waterman alignment [default= %default]", default=-4, type="int")
	group.add_option("-4", "--gapextendscore", action="store", dest="gap_extend_score", help="Score for extending a gap in Smith-Waterman alignment [default= %default]", default=-3, type="int")
	group.add_option("-K", "--kmer", action="store", dest="kmer", help="kmer size [default= %default]", default=13, type="int")
...
#		
	...
	def runSMALT(self, ref, bashfile):	
		...
		if self.domapping:
			...
			if self.pairedend:
				if options.maprepeats:
					print >> bashfile, SMALT_DIR+" map -S "+options.scorspec+" -y "+str(options.nomapid)+" -x -r 0 -i", options.maxinsertsize, " -j", options.mininsertsize, " -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
					cmdline="map -S "+options.scorspec+" -y "+str(options.nomapid)+" -x -r "+str(randrange(1,99999))+" -i", options.maxinsertsize, " -j", options.mininsertsize, " -f "+smaltoutput+"  -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
				else:
					if newsmalt:
						rbit=" -r -1"
					else:
						rbit=""
					print >> bashfile, SMALT_DIR+" map -S "+options.scorspec+" -y "+str(options.nomapid)+rbit+" -x -i", options.maxinsertsize, " -j", options.mininsertsize, " -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
					cmdline="map -S "+options.scorspec+" -y "+str(options.nomapid)+rbit+" -x -i", options.maxinsertsize, " -j", options.mininsertsize, " -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+"_1.fastq", self.fastqdir+self.name+"_2.fastq"
			else:
				if options.maprepeats:
					print >> bashfile, SMALT_DIR+" map -S "+options.scorspec+" -S "+options.scorspec+" -y "+str(options.nomapid)+" -x -r 0 -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+".fastq"
					cmdline="map -S "+options.scorspec+" -S "+options.scorspec+" -y "+str(options.nomapid)+" -x -r "+str(randrange(1,99999))+" -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+".fastq"
				else:
					if newsmalt:
						rbit=" -r -1"
					else:
						rbit=""
					print >> bashfile, SMALT_DIR+" map -S "+options.scorspec+" -y "+str(options.nomapid)+rbit+" -x -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+".fastq"
					cmdline="map -S "+options.scorspec+" -y "+str(options.nomapid)+rbit+" -x -f "+smaltoutput+" -o "+self.runname+"/tmp1."+smaltoutputsuffix, tmpname+".index", self.fastqdir+self.name+".fastq"
			if not newsmalt:
				print >> bashfile, SAMTOOLS_DIR+"samtools view -b -S",self.runname+"/tmp1.sam -t "+ref+".fai >", self.runname+"/tmp1.bam"
		...
		if options.GATK:
			...
			print >> bashfile, SAMTOOLS_DIR+"samtools view -b -o", self.runname+'/tmphead.bam' ,"-H", self.runname+"/"+self.name+".bam"
			print >> bashfile, SAMTOOLS_DIR+'samtools merge -r -h ', self.runname+'/tmphead.sam', self.runname+"/tmp1.bam", self.runname+"/"+self.name+".bam", self.runname+'/tmphead.bam'
...
if __name__ == "__main__":
	...
	options.scorspec="match="+str(options.match_score)+",subst="+str(options.subst_score)+",gapopen="+str(options.gap_open_score)+",gapext="+str(options.gap_extend_score)
...
#		pools[count].readlength=options.readlength
	...
	if options.program=='BWA':
		...
		if options.human:
			...
			os.system(SMALT_DIR+" index -k "+str(options.kmer)+" -s 1 "+tmpname+".index "+options.ref)
